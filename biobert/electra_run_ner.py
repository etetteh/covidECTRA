...
flags.DEFINE_integer(
    "embedding_size", 128, # change to 768 for base model
)
...
class NerProcessor(DataProcessor):
    ...
    def _create_example(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            text = e_tokenization.convert_to_unicode(line[1])
            label = e_tokenization.convert_to_unicode(line[0])
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        scaffold_fn = None
        if init_checkpoint:
            (assignment_map, initialized_variable_names) = e_modeling.get_assignment_map_from_checkpoint(tvars,init_checkpoint)
